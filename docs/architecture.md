...
Startup initializes/migrates schemas and reads the catalog; it does not overwrite catalog bindings from provider lists or perform provider discovery. Empty storage serves an empty editor until explicitly populated. Ordinary default profiles are seeded once when compatible voices exist; deleted profiles never reappear. The remaining legacy capability projection in `providers/options.py` serves explicit workshop validation, not profile/editor model selection.

`voice_migrations.py` converts earlier raw/system profiles and multi-model catalogs to the final structure in one forward migration. Compatible identities/settings/timestamps are retained. Authorized cleanup removes unresolved or incompatible legacy voice/profile records and reports local IDs/reasons; no recurring cleanup runs during sync. Generation snapshots, audio/images and retained reference bundles are independent. Schema rollback requires the matching database backup as well as the older code.
